...
async fn tunnel_loop(server_url: String, token: String) {
    ...
    loop {
        ...
        match connect_async(request).await {
            Ok((ws_stream, _)) => {
                ...
                if let Some(cmd) = command_received {
                    ...
                    // [BUG-4 FIX] Normal completion (bridged successfully): reconnect immediately without delay.
                    continue;
                }
            }
            Err(e) => {
        ...
        // Sleep on connection errors or if disconnected without receiving a command
